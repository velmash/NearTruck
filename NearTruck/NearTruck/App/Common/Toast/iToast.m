...
- (void) show:(iToastType) type {
    ...
    label.textColor = [UIColor whiteColor];     //[UIColor whiteColor];
    label.font = font;
    label.text = text;
...
+ (void) showToast:(NSString *) text{
    [[[[iToast makeText:text] setGravity:iToastGravityBottom] setDuration:iToastDurationNormal] show];
}
...
+ (iToastSettings *) getSharedSettings{
    if (!sharedSettings) {
        ...
        sharedSettings.bgRed = 71/255.0;
        sharedSettings.bgGreen = 71/255.0;
        sharedSettings.bgBlue = 71/255.0;
